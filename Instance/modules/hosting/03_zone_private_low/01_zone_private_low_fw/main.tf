...
resource "azurerm_virtual_machine" "vm" {
  ...
  os_profile {
    ...
    custom_data    = "${file("${path.module}/scripts/configuration_fw.sh")}"
...
resource "azurerm_virtual_machine_extension" "ciap-hosting-low-extension" {
  name                = "${var.hostname}-low-${count.index}"
  location            = "${var.location}"
  resource_group_name = "${var.resource_group}"
